...
#warning This probably needs to be MORE UTF8 aware now
...
//	wbuf[fl_utf2unicode(findIn, strlen(findIn), wbuf)] = 0;
	...
	wlen++;
	unsigned short *wbuf = (unsigned short*)malloc(sizeof(unsigned short)*wlen);
	wlen = fl_utf8toUtf16(findIn, strlen(findIn), wbuf, wlen);
	wbuf[wlen] = 0;
